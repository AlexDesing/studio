...
export default function AffirmationsPage() {
  ...
  const [isDeleteVBConfirmOpen, setIsDeleteVBConfirmOpen] = useState(false);
  const [visionBoardItemToDeleteId, setVisionBoardItemToDeleteId] = useState<string | null>(null);
  ...
  const confirmDeleteVisionBoardItem = (itemId: string | undefined) => {
    if (!itemId) return;
    setVisionBoardItemToDeleteId(itemId);
    setIsDeleteVBConfirmOpen(true);
  };

  const executeDeleteVisionBoardItem = async () => {
    if (!currentUser || !visionBoardItemToDeleteId) return;
    try {
        await deleteVisionBoardItem(currentUser.uid, visionBoardItemToDeleteId);
        ...
        setVisionBoardItemToDeleteId(null);
    }
  };
  ...
  return (
    <div className="w-full max-w-5xl px-6 ml-[calc(max(0px,50vw-40rem))] space-y-12">
      <section>
          ...
          <Card className="shadow-2xl bg-card/90 backdrop-blur-sm border-secondary/50 rounded-xl overflow-hidden relative pb-16 min-h-[600px] flex flex-col">
            ...
            >
                ...
                <div className="absolute top-4 right-4 flex space-x-2">
                    ...
                    <Button variant="destructive" size="icon" className="h-9 w-9 bg-destructive/80 hover:bg-destructive" onClick={() => confirmDeleteVisionBoardItem(currentVisionItem.id)}>
                        <Trash2 size={18} />
      ...
      <Dialog open={isVisionBoardDialogOpen} onOpenChange={setIsVisionBoardDialogOpen}>
        ...
        <AlertDialog open={isDeleteVBConfirmOpen} onOpenChange={setIsDeleteVBConfirmOpen}>
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>¿Estás absolutamente segura?</AlertDialogTitle>
              <AlertDialogDescription>
                Esta acción no se puede deshacer. Esto eliminará permanentemente esta visión de tu diario.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              <AlertDialogCancel onClick={() => { setIsDeleteVBConfirmOpen(false); setVisionBoardItemToDeleteId(null); }}>Cancelar</AlertDialogCancel>
              <AlertDialogAction onClick={executeDeleteVisionBoardItem}>Sí, eliminar visión</AlertDialogAction>
            </AlertDialogFooter>
          </AlertDialogContent>
        </AlertDialog>
    </div>
