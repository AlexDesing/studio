...
export default function RoutinesPage() {
  ...
  const handleSaveRoutine = async () => {
    ...
    if (routineStartDate) {
      ...
      // Ensure date is created in local timezone then converted if needed by Firestore
      const localDate = new Date(year, month - 1, day);
      routineData.startDate = Timestamp.fromDate(localDate);
    } else {
  ...
  const confirmDeleteAndCloseDialog = async () => {
    if (!currentUser || !routineToDeleteId) return;
    try {
        await deleteRoutine(currentUser.uid, routineToDeleteId);
      ...
      <Dialog open={isFormDialogOpen} onOpenChange={setIsFormDialogOpen}>
        ...
        <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
          <AlertDialogContent>
            ...
            <AlertDialogFooter>
              <AlertDialogCancel onClick={() => {setIsDeleteDialogOpen(false); setRoutineToDeleteId(null);}}>Cancelar</AlertDialogCancel>
