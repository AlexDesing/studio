...
export default function SettingsPage() {
  ...
  useEffect(() => {
    if (currentUser) {
      ...
      // Ensure preferences and theme are defined before accessing
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      // Use localPhotoURL which is confirmed to be string | null
    ...
    <div className="w-full max-w-5xl px-6 ml-[calc(max(0px,50vw-40rem))] space-y-10"> 
      ...
      <Card className="shadow-lg">
        <CardHeader>
          ...
          <CardDescription>Gestiona qué alertas deseas recibir.</CardDescription>
        </CardHeader>
        <CardContent>
            {/* Link to the dedicated notifications page */}
            <p className="text-muted-foreground">
              Puedes gestionar tus preferencias de notificación detalladas en la sección de{' '}
              <Link href="/app/notifications" className="text-highlight-purple hover:underline">
                Notificaciones
              </Link>
              .
            </p>
